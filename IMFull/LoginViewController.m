...
- (IBAction)resignAndLogin:(id)sender
{
    ...
    if (tf.tag == 1) {
        ...
        NSPredicate *pred = [NSPredicate predicateWithFormat:@"(name == %@ && password == %@)", [usernameField text], [passwordField text]];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self performSegueWithIdentifier:@"LoginSegue" sender:intNum];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    IMMainTable *questionlist = (IMMainTable *)[[navController viewControllers] lastObject];
    questionlist.managedObjectContext = managedObjectContext;
}
